<template>
    <v-card class="mx-auto mb-4" max-width="344" v-for="item in datavalue" :key="item">
        
        <v-card-item>
            <v-card-actions>
            <v-spacer></v-spacer>

            <v-btn size="medium" color="surface-variant" variant="text" icon="mdi-delete" @click="$store.dispatch('deleteWork',item.id)"></v-btn>
...
export default {
    ...
    data: (props) => ({
        datavalue: Object.assign({}, props.workProp)

    }),
    watch: {
        workProp() {
            this.value = Object.assign({}, props.workProp);
        }
